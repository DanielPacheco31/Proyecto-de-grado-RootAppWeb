...
def descargar_factura(request: HttpRequest, compra_id: int) -> HttpResponse:
    ...
    try:
        ...
        try:
            ...
            messages.error(request, f'error al realizar la factura: {str(e)}')
    ...
    try:
        ...
        messages.error(request, f'Error al ubicar al PDF: {str(e)}')
        return render(request, "facturas/detalle_compra.html", {"compra": compra,"factura": factura})
...
def cancelar_compra(request, compra_id):
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            if request.headers.get('X-Requested-With') == 'XMLHttpRequest':
                return JsonResponse({'success': False,'message': f'Error al cancelar la compra: {str(e)}'})
